...
\begin{document}
	...
	\begin{frame}[fragile]{Observation 2.6 \textit{ is implied by Lemma 2.5 }}
		...
		\only<1>{ \(V = \{a, b, c, d\}\) \qquad \(E = \{(a, b), (b, c), (c, d), (d, a)\}\) \\
			\(\Rightarrow a < b \land b < c \land c < d \land d < \dots \Rightarrow \text{ no total order } \Rightarrow \text{ cyclic }\) }
	 	\only<2-3>{\(V = \{a, b, c, d\}\) \qquad \((E\cup rev(F))\setminus F = \{(a, b), (b, c), (c, d), \color{blue}(a, d)\}\) \\
	...
	\begin{frame}[fragile]{Lemma 2.7 ``\(\Leftarrow\)'' \--- Proof}
		...
		\only<+->{\[ \text{if } \exists F' : \abs{F'} < \abs{F} \land F' \in min \left\{ F' \subset E(G) | G\circledast F' \text{ is acyclic} \right\} \]}
		\only<+->{\[ \stackrel{\text{Lemma 2.7 } \Rightarrow}{\Rightarrow} F' \in min \left\{ F' \subset E(G) | F' \text{ is a Feedback Arc Set of } G \right\} \]}
		\only<6->{\textbf{Contradiction!}}
	\end{frame}
	...
	\begin{frame}[fragile]{Reduction FAST.1 - Polynomial time}
		%TODO: Pseudocode
		FAST.1 is a computable function in time bounded by
		\[ \mathcal{O}(\abs{E(G)}^3) + \mathcal{O}(\abs{E(G)}^4) + \mathcal{O}(1) = \mathcal{O}(\abs{E(G)}^4) \]
	\end{frame}
