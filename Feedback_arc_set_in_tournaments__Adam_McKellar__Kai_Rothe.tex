...
\begin{document}
	...
	\begin{frame}[fragile]{Lemma 2.7 ``\(\Leftarrow\)'' \--- Proof}
		...
		\only<+->{\[ \text{if } \exists F'' : \left( \abs{F''} < \abs{F} \right) \land \left( F'' \in \min \left\{ F' \subset E(G) | F' \text{ is a FAS of } G \right\} \right) \]}
	...
	\begin{frame}[fragile]{Reduction FAST.1 - Safeness}
		...
		\only<2>{ 
		\begin{align*}
		& \mathbb{A}: \exists e \in E(G) \text{ contained in at least } k+1 \text{ triangles } \\
	...
	\begin{frame}[fragile]{Reduction FAST.2 - Safeness}
		...
		\uncover<2->{ \includegraphics[height=0.3\paperheight]{images/FAST_2/GraphVCircle.pdf} }
		\end{center}
